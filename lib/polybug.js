...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'polybug:continue': () => this.debugger && this.debugger.continue(),
      'polybug:step': () => this.debugger && this.debugger.step(),
      'polybug:next': () => this.debugger && this.debugger.next(),
      'polybug:return': () => this.debugger && this.debugger.return(),
      'polybug:restart': () => this.debugger && this.debugger.restart(),
    }));
    this.state = state;
  },
  ...
  debug(directory, commandLine) {
    if (!this.panel) {
      ...
      leftElement.appendChild(this.toolbar);
      this.addButton('<svg width="16" height="16"><polygon points="4,1 11,9 4,16"/></svg>', 'Continue', () => this.debugger.continue());
      this.addButton('<svg width="16" height="16"><rect x="0" y="1" width="2" height="15"/><rect x="3" y="1" width="2" height="15"/><polygon points="7,1 14,9 7,16"/></svg>', 'Next', () => this.debugger.next());
      this.addButton('<svg width="16" height="16"><rect x="2" y="1" width="2" height="15"/><polygon points="6,1 13,9 6,16"/></svg>', 'Step', () => this.debugger.step());
      this.addButton('<svg width="16" height="16"><polygon points="0,1 7,9 0,16"/><polygon points="8,1 15,9 8,16"/></svg>', 'Return', () => this.debugger.return());
      this.addButton('<svg width="16" height="16"><path d="M3 7 A 6 6, 0, 1, 0, 7 3" stroke="black" fill="transparent"/><polygon points="4,3 9,0 9,6"/></svg>', 'Restart', () => this.debugger.restart());
      this.output = document.createElement('pre');
      this.output.className = 'output';
      leftElement.appendChild(this.output);
      this.input = document.createElement('input');
      this.input.placeholder = '>>';
      this.input.className = 'native-key-bindings';
      leftElement.appendChild(this.input);
      element.className = 'polybug-panel';
      this.panel = atom.workspace.addBottomPanel({item: element, visible: false});
      this.stack = new StackTrace(rightElement);
    ...
    if (this.panel.isVisible()) {
      ...
      this.stack.attachDebugger(this.debugger);
    }
  }
